...
test('GET / returns discovery plus the three readings', async () => {
  ...
  assert.equal(body.worker, 'uuidna-lean')
...
test('GET /seat /width /hologram and well-known', async () => {
  ...
  assert.equal(disc.donate, 'https://revolut.me/ceccec?note=https%3A%2F%2Flean.uuidna.com')
...
test('nav sidebar search superpositions are computed upon request', async () => {
  ...
  assert.equal(sup.superpositions[0]!.angles.hue, 40)
...
test('http and www 301 to https apex', async () => {
  const http = await handleQpuFetch(new Request('http://lean.uuidna.com/seat'))
  assert.equal(http.status, 301)
  assert.equal(http.headers.get('location'), 'https://lean.uuidna.com/seat')
  const www = await handleQpuFetch(new Request('https://www.lean.uuidna.com/'))
  assert.equal(www.status, 301)
  assert.equal(www.headers.get('location'), 'https://lean.uuidna.com/')
})
