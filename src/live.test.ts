...
test('live snapshot recomputes metrics and walks every rung', () => {
  ...
  assert.equal(a.k, 0)
  assert.equal(b.k, 0)
  assert.equal(qpuLiveKOf(1000), 7)
  assert.equal(qpuLiveKOf(2000), 1)
  assert.equal(qpuLiveKOf(3000), 8)
  assert.deepEqual(a.walk, [0, 7, 1, 8, 2, 9, 3, 10, 4, 11, 5, 12, 6, 13])
  assert.equal(qpuLiveOf(1000).hop, 0)
...
test('GET /live is occupancy now; MCP qpu_live matches', async () => {
  const res = await handleQpuFetch(new Request('https://lean.uuidna.com/live'))
