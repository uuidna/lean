#!/usr/bin/env node
// gen-readme — README, MANUAL, CITATION.cff, and .zenodo.json. Never hand-edit those files.
...
/** THE ARCHIVE METADATA IS THE CITATION, IN ZENODO'S SHAPE. Measured 2026-09-12: this repository carried a
 *  hand-written .zenodo.json titled as the @uuidna/qpu worker, with donation links in the creator's affiliation,
 *  "quantum supremacy" and SDK names as keywords, and membership requests to three quantum-software communities —
 *  beside a generated CITATION.cff that named the package correctly. Two files, one work, two stories. The first
 *  release would have minted the wrong one permanently. Now both are derived from package.json and standing.ts. */
export function zenodoOf(): string {
  const pkg = pkgOf()
  ...
  const zenodo = {
    title,
    description:
      `${(pkg.description ?? title).replace(/\.\s*$/, '')}. Cloudflare Worker serving JSON readings of an empty QPU seat, BindingPoint width ` +
      `(CPU, GPU, RAM, CACHE, STORAGE), and hologram planes. Not a hardware quantum processing unit. ` +
      `Proofs live on uuidna (DOI ${UUIDNA_DOI}). Licence ${licenseLabelOf(licenseOf(pkg))}.`,
    upload_type: 'software',
    access_right: 'open',
    license: licenseOf(pkg).toLowerCase(),
    language: 'eng',
    version: pkg.version,
    publication_date: DATE_RELEASED,
    creators: [{ name: `${family}, ${given}`, orcid: ORCID, affiliation: scopeOf(pkg.name) }],
    keywords: pkg.keywords ?? [],
    communities: [{ identifier: scopeOf(pkg.name) }],
    related_identifiers: [
      { identifier: live, relation: 'isIdenticalTo', resource_type: 'software' },
      { identifier: github, relation: 'isSupplementTo', resource_type: 'software' },
      { identifier: `https://www.npmjs.com/package/${pkg.name}`, relation: 'isIdenticalTo', resource_type: 'software' },
      { identifier: UUIDNA_DOI, relation: 'isDerivedFrom', resource_type: 'software' },
    ],
    references: [`${apaNameOf()} (${yearReleasedOf()}). ${UUIDNA_TITLE} [Computer software]. ${UUIDNA_DOI_URL}`],
    notes: `Licence ${licenseLabelOf(licenseOf(pkg))} — ${ccUrlOf(licenseOf(pkg))}. Parent software DOI ${UUIDNA_DOI}. Metadata generated by gen-readme from package.json.`,
  }
  return `${JSON.stringify(zenodo, null, 2)}\n`
}
...
async function main(): Promise<void> {
  ...
  writeFileSync(join(ROOT, '.zenodo.json'), zenodoOf())
  ...
  console.log(`✓ CITATION.cff .zenodo.json README.md MANUAL.md docs/paper.md docs/manual.md · tests ${receipt.pass}/${receipt.tests}${receipt.fail ? ` · fail ${receipt.fail}` : ''}`)
